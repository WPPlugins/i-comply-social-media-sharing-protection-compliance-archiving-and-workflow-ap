<?php
/* Plugin Name: i-Comply
 * Plugin URI: http://wordpress.org/plugins/icomply/
 * Description: Publish posts automatically from your blog to i-Comply site.
 * Author: Notetech
 * Version: 1.0.1
 */
?>
...
include_once ('icomply_setup.php');
include_once ('icomplyapi.php');
...
{
	include_once ('icomplysettings.php');
	die();
}
...
<div id="dialog" title="Send draft to i-Comply?">
      <p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>Send draft to i-Comply for approval now?</p>
</div>
<?php 
    }
}
...
function icomply_meta_box() {
    ...
    if (get_post_type($post) == 'post') {
        ...
        ?>
        <div>
        	<img src="<?php echo plugins_url( 'icomply', 'icomply' ).'/icomply.png';?>" height="25" />
       			<?php 
        			global $wpdb;
        			$user_ID = get_current_user_id();
        			$table_name = $wpdb->prefix . "icomplydetails";
        			$user = $wpdb->get_row("SELECT * FROM $table_name WHERE userid = $user_ID");        
        			$url= admin_url( 'options-general.php?page=icomplysettings' , 'http' );        
    		...
    		{?> | <a id="viewedit"  href="javascript:toggle_edit();" >Edit</a><?php }?></span>         			        
        			...
        			else
        			{
        			     ...
        			     echo 'Set Your i-Comply details&nbsp;</span><a id="displayText" style="float:right;padding-top:4px;" href="'.$url.'">Click</a>';
...
<?php 
    		...
    		{ 
	    		...
	    		<tr>
	                <td>
	                    <table width="100%" cellpadding="0" cellspacing="0">
	                             <tr align="left"><td valign="middle" width="90%"><b>Custom Message</b></td><td><img  src="<?php echo plugins_url( 'icomply', 'icomply' ).'/blue_question_mark.jpg'; ?>" title="{POST_TITLE}=>Title of post&#013;{PERMALINK}=>Permalink to your post"  onclick="alert(' {POST_TITLE}=>Title of post \n {PERMALINK}=>Permalink to your post');"  /></td> </tr>
    ...
    if($user)
    { ?>
    <div style="text-align:right;"><a href="http://icomply02.isocialsmart.com/icomply-web/queue" style="font-weight:normal;font-size:12px;" target="_blank">Open i-Comply</a></div>
<?php 
    }
    echo '</div>';
    }
...
function sb_post_validation() {
    ...
    {       
        ...
        {
            $url= plugins_url( 'icomplysettings.php' , __FILE__ ) ;
            ?>
             <script type="text/javascript">
...
function register_icomplysetting_submenu_page() {
    add_submenu_page( 'options-general.php', 'i-Comply Settings', 'i-Comply Settings', 'manage_options', 'icomplysettings', 'icomplysetting_submenu_page_callback' );
}
